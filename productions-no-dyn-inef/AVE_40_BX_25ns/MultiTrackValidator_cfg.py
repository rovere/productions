...
EOS_REPO = '/store/group/phys_tracking/samples_720pre4/DIGI-NO-PIX-DYN-INEF/AVE_%s/TTbar/' % JOB_LABEL
...
readFiles.extend( ['/store/group/phys_tracking/samples_720pre4/RECO-NO-PIX-DYN-INEF/AVE_%s/TTbar/step3_%sNODUP_RAW2DIGI_L1Reco_RECO_DQM.root' %(JOB_LABEL,JOB_LABEL)] );
...
process.multiTrackValidator.runStandalone = cms.bool(False)
...
process.load('Validation.RecoVertex.PrimaryVertexAnalyzer4PUSlimmed_cfi')
process.load('Validation.RecoVertex.PrimaryVertexAnalyzer4PUSlimmed_Client_cfi')
process.load('Configuration.StandardSequences.EDMtoMEAtJobEnd_cff')
process.dqmSaver.workflow = cms.untracked.string('/Validation/Release720pre4-NoDynInef/%s' % JOB_LABEL)
...
process.validation = cms.Sequence(
    ...
    process.vertexAnalysisSequence * 
    process.postProcessorVertexStandAlone
    )
...
process.schedule = cms.Schedule(
    process.p, process.dqmsave_step
    )
